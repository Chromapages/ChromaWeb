...
export type OfferPageData = {
  ...
  outcomePrompt?: string | null;
  shortTransformation?: string | null;
  primaryGoal?: string | null;
  bestFor?: string | null;
  fitReasons?: Array<string | null> | null;
  problem?: string | null;
  solution?: string | null;
  deliverables?: string[] | null;
  investmentRange?: string | null;
  investmentTiers?: InvestmentTier[] | null;
  timeline?: string | null;
...
const OfferCta = ({cta, analyticsLocation, inverse = false}: {cta?: OfferPageData["cta"]; analyticsLocation: string; inverse?: boolean}) => {
...
export const OfferPage = ({data, fallbackTitle}: OfferPageProps) => {
  ...
  const investmentValue = data?.investmentRange
    ?? (data?.investmentTiers && data.investmentTiers.length > 0
      ? data.investmentTiers.map((tier) => `${tier.label}: ${tier.amount}`).join(" · ")
      : null);

  const metadataItems = [
    ...(investmentValue ? [{label: "Investment", value: investmentValue}] : []),
  ...
  return (
    <main id="main-content" tabIndex={-1}>
      ...
      {problem || solution ? (
        <section className="bg-canvas">
          <div className="mx-auto grid w-full max-w-main gap-px px-6 py-20 md:grid-cols-2 lg:px-10 lg:py-28">
            {problem ? (
              <article className="border-t border-ink/15 pt-6 md:pr-10">
            ...
            {solution ? (
              <article className="border-t border-ink/15 pt-6 md:pl-10">
      ...
      {data?.deliverables?.length ? (
        <EditorialSection number="01 / Deliverables" title="What the engagement includes">
      ...
      {data?.investmentRange || (data?.investmentTiers && data.investmentTiers.length > 0) || data?.timeline ? (
        <section className="bg-indigo text-canvas">
          <div className="mx-auto grid w-full max-w-main gap-px px-6 py-20 md:grid-cols-2 lg:px-10 lg:py-28">
            {data.investmentTiers && data.investmentTiers.length > 0 ? (
              <article className="border-t border-canvas/25 pt-6 md:pr-10">
                <p className="text-xs font-semibold tracking-[0.18em] text-canvas/85 uppercase">Investment structure</p>
                {data.investmentRange ? (
                  <p className="mt-4 font-display text-3xl font-semibold tracking-[-0.03em]">{data.investmentRange}</p>
                ) : null}
                <div className="mt-5 space-y-4">
                  {data.investmentTiers.map((tier) => (
                    <div key={tier.label} className="border-l-2 border-teal-300/60 pl-3">
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <span className="font-display text-base font-semibold text-canvas">{tier.label}</span>
                        <span className="font-display text-lg font-bold text-teal-300">{tier.amount}</span>
                      </div>
                      {tier.description ? <p className="mt-1 text-sm text-canvas/75">{tier.description}</p> : null}
                      {tier.timeline ? <p className="mt-0.5 text-xs text-canvas/60">Timeline: {tier.timeline}</p> : null}
                    </div>
                  ))}
                </div>
              </article>
            ...
            {data.timeline ? (
              <article className="border-t border-canvas/25 pt-6 md:pl-10">
      ...
      {data?.deliverySteps?.length ? (
        <EditorialSection number="02 / Delivery" title="A controlled path to launch" dark>
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {data.relatedIndustries.map((industry) => (
              <li key={industry?.slug ?? industry?.title}>
                {industry?.slug ? (
                  <Link className="block border-b border-ink/15 py-4 font-display text-xl font-semibold hover:text-teal focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-teal" href={`/industries/${industry.slug}`}>{industry.title}</Link>
                ) : (
                  <span className="block border-b border-ink/15 py-4 font-display text-xl font-semibold">{industry?.title}</span>
                )}
              </li>
            ))}
          </ul>
