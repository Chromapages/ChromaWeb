...
export function SiteHeader({logoUrl}: {logoUrl?: string | null}) {
  ...
  return (
    <header className="sticky top-0 z-40 h-14 border-b border-ink/15 bg-canvas lg:h-[72px]" ref={headerRef}>
      <nav aria-label="Primary navigation" className="relative mx-auto grid h-full w-full max-w-content-wide grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 px-3 sm:px-6 lg:grid-cols-[minmax(10rem,1fr)_auto_minmax(10rem,1fr)] lg:px-10">
        ...
        <div className="col-start-2 flex items-center justify-end gap-3 lg:col-start-3">
          <Link className="inline-flex h-8 items-center justify-center rounded-lg bg-teal px-3 text-[11px] font-semibold text-canvas transition-colors hover:bg-indigo focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-indigo lg:h-10 lg:px-5 lg:text-sm" href="/contact" onClick={closeMenu}>
          ...
          <Link
            ...
            className="hidden h-10 items-center justify-center rounded-lg border border-ink/25 px-4 text-sm font-semibold text-ink transition-colors hover:border-teal hover:bg-ink/5 hover:text-teal focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-teal lg:inline-flex"
        ...
        {isMenuOpen ? (
          <div className="absolute inset-x-0 top-full border-b border-ink/15 bg-canvas shadow-[0_12px_24px_rgba(15,17,21,0.08)] lg:hidden" id="compact-navigation">
            <ul className="mx-auto grid max-w-main gap-1 px-3 py-4 sm:px-6">
              ...
              <li>
                <Link className="block rounded-sm px-3 py-3 text-base font-semibold text-teal hover:bg-ink/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal" href="/login?redirect=/portal" onClick={closeMenu}>
                  Client Portal →
                </Link>
              </li>
            </ul>
